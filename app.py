...
def create_carousel_template(restaurants):
    ...
    for restaurant in restaurants:
        name = restaurant["displayName"]["text"]
        rating = restaurant.get("rating", "無評分")
        address = restaurant.get("formattedAddress", "無地址資訊")
        place_id = restaurant["id"]
        maps_url = f"https://www.google.com/maps/place/?q=place_id:{place_id}"
        ...
        if "photos" in restaurant and len(restaurant["photos"]) > 0:
            ...
            photo_url = "https://via.placeholder.com/400x200.png?text=No+Image"  # 預設圖片
        
        # 建立 Carousel Column
        column = CarouselColumn(
            thumbnail_image_url=photo_url,  # 餐廳圖片
            title=name,  # 餐廳名稱
            text=f"⭐{rating}\n{address}",  # 評分和地址
            actions=[
                URIAction(
...
def handle_message(event):
    ...
    if lat and lng:
        restaurants = search_nearby_restaurants(lat, lng)
        if restaurants:
            carousel_template = create_carousel_template(restaurants)
